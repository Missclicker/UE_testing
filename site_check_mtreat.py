...
def main(check_fw, check_e1, mtu_check, last_site, set_loops):
    ...
    if check_e1 == 'all':
        ...
        any_mux_up = all_mux_check(site['infra']['MUX_MGMT'], any_check=True)
        for rtr in site['rtrs']:
            print(f"working with {rtr['Hostname']}")
            mux_check_e1(rtr, site['infra']['MUX_MGMT'], via_shut=any_mux_up)
            if any_mux_up:
                # also sets loopbacks
                unshut_e1(rtr)
...
@click.option('-e1', '--check_e1', default='direct', help='How to check e1 -accepted values are: direct, all, no')
@click.option('-l', '--last_site', is_flag=True, default=False, help='Use last cached site')
def mux_check(check_e1, last_site):
    # TODO check type
...
@click.option('-s', '--set_loops', is_flag=True, default=False, help='Set loops. Default - remove loops(no flag)')
@click.option('-l', '--last_site', is_flag=True, default=False, help='Use last cached site')
def manage_loop(set_loops, last_site):
...
@cli.command()
@click.option('-s', '--shut', is_flag=True, default=False, help='Set loops. Default - remove loops(no flag)')
@click.option('-l', '--last_site', is_flag=True, default=False, help='Use last cached site')
def shut_cem(shut, last_site):
    """Set/remove loops on all E1 on site"""
    site = click_site(last_site)
    if 'infra' in site.keys():
        for rtr in site['rtrs']:
...
@cli.command()
@click.option('-l', '--last_site', is_flag=True, default=False, help='Use last cached site')
@click.option('-fw', '--firewalls', is_flag=True, default=False, help='Use last cached site')
def ssh_all(last_site, firewalls):
    site = click_site(last_site)
    if firewalls and 'infra' in site.keys():
        for ip in site['infra']['FW_MGMT'].keys():
            subprocess.Popen(f"{PATH / 'putty.exe'} -ssh -pw C!sco123 admin@{ip} 22",
                             stdin=None, stdout=None, stderr=None, close_fds=True)
    elif 'rtrs' in site.keys():
        print(f'Connecting to {site["name"]}')
        for rtr in site['rtrs']:
            subprocess.Popen(f"{PATH / 'putty.exe'} -ssh -pw C!sco123 root@{rtr['Loopback0']} 22",
                             stdin=None, stdout=None, stderr=None, close_fds=True)
    else:
        print('No IPs found for this site, run MAIN first')
...
def choose_sites():
    ...
    while True:
        choose = input('Enter site name> ').strip().lower()
        ...
        # replace letters
        choose = re.sub('[іиєїеь]', '.', choose)
        # drop single letters
        choose = [x for x in choose.split() if len(x) > 1]
        pattern = '|'.join(['.*'.join(x) for x in itertools.permutations(choose, len(choose))])
        site_matches = [x for x in st if re.search(pattern, x.lower())]
        if not site_matches:
            print('no matches, re-enter site name')
...
# def all_mux_mtread(muxes, searching_status='11'):
#     threads= []
#     with ThreadPoolExecutor(max_workers=5) as executor:
#         for ip in muxes:
#             if muxes[ip]['eif'] == '':
#                 threads.append(executor.submit(get_mux_status, ip))
#
#         for task in as_completed(threads):
#             ip, status = task.result()
#             #print(ip, status)
#             if status == searching_status:
#                 return ip
#             elif not status:
...
def mux_check_e1(rtr, muxes, via_shut=False):
    site_rtr = {
        'device_type': 'cisco_ios',
        'host': rtr['Loopback0'],
        'username': LOGIN,
    ...
    with netmiko.ConnectHandler(**site_rtr) as ssh:
        ...
        for e1 in rtr['e1']:
            ...
            sleep(3)
            #             ip = all_mux_mtread(muxes, searching_status)
            ip = all_mux_check(muxes, searching_status)
            if ip:
                if muxes[ip]['eift'] != f"{rtr['Hostname']} {e1}":
                    print(f'check E1 connect on {muxes[ip]["dev"]}')
                    muxes[ip]['eif'] = '!!!' + f"{rtr['Hostname']} E1-{e1}"
                else:
                    print(f"{muxes[ip]['dev']} <-> {rtr['Hostname']} E1-{e1} - OK")
...
def manage_cem_ifs(rtr, shut=False):
    if shut:
        shut = ''
    else:
        shut = 'no '
...
def print_infra(infr, check_fw=True):
    ...
    for i in infra_list:
        if 'MUX' in i[0]:
            print(f'{i[0]:<10s}{i[1]:<16s}{i[2]:<24s}{i[3]!s:<8s}{i[4]:<36s}{infr[i[0]][i[1]]["eif"]:<25s}  ' +
                  infr[i[0]][i[1]]['eift'])
